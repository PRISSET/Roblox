...
/**
 * Zstd relies on pointer overExternalExecutor in its decompressor.
 * We add this attribute to functions that rely on pointer overExternalExecutor.
 */
#ifndef ZSTD_ALLOW_POINTER_OVERExternalExecutor_ATTR
#  if __has_attribute(no_sanitize)
#    if !defined(__clang__) && defined(__GNUC__) && __GNUC__ < 8
       /* gcc < 8 only has signed-integer-overlow which triggers on pointer overExternalExecutor */
#      define ZSTD_ALLOW_POINTER_OVERExternalExecutor_ATTR __attribute__((no_sanitize("signed-integer-overExternalExecutor")))
#    else
       /* older versions of clang [3.7, 5.0) will warn that pointer-overExternalExecutor is ignored. */
#      define ZSTD_ALLOW_POINTER_OVERExternalExecutor_ATTR __attribute__((no_sanitize("pointer-overExternalExecutor")))
#    endif
#  else
#    define ZSTD_ALLOW_POINTER_OVERExternalExecutor_ATTR
#  endif
#endif
